...
The backend also includes a torch-only team graph layer in `scripts/gnn_graph_layer.py`.
It builds a completed-match interaction graph from `data/temp_extract` and closed SQLite
results, trains one model per league, and writes gated checkpoints to `model/gnn`.

Train or refresh the graph layer:

```sh
python scripts/gnn_graph_layer.py --league ALL --epochs 120 --min-matches 120
```

The generated `model/gnn/gnn_training_report.json` records validation accuracy,
baseline accuracy, lift, and whether each league is enabled. Live predictions only
blend the GNN probabilities when that league's metadata is enabled; the blend is
capped so the graph model acts as an accuracy feature layer instead of replacing
the calibrated core model.

External open-source training data can be imported from the xgabora Club Football
Match Data dataset after downloading `data/Matches.csv` into
`external_data/xgabora/Matches.csv`:

```sh
python scripts/import_xgabora_dataset.py
python scripts/gnn_graph_layer.py --league ALL --epochs 120 --min-matches 500
python scripts/gnn_walk_forward_validation.py --league ALL --epochs 40
```
